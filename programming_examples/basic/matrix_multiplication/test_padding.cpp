...
int main(int argc, const char *argv[]) {
  ...
  /* Update M, N, and K with padded sizes*/ 
  int finalM = ceil_div(M, m*4) * (m*4);
  int finalK = ceil_div(K, k) * k;
  int finalN = ceil_div(N, n*4) * (n*4);

  std::cout << "Final M="<<finalM<<std::endl;
  std::cout << "Final K="<<finalK<<std::endl;
  std::cout << "Final N="<<finalN<<std::endl;
    
  int A_finalVolume = finalM * finalK;
  int B_finalVolume = finalN * finalK;
  int C_finalVolume = finalM * finalN;
    
  int A_VOLUME = M * K;
  int B_VOLUME = N * K;
  // int C_VOLUME = M * N;
    int C_VOLUME = C_finalVolume;


  size_t A_SIZE = (A_finalVolume * sizeof(A_DATATYPE));
  size_t B_SIZE = (B_finalVolume * sizeof(B_DATATYPE));
  ...
  /* Pad A */
    std::vector<A_DATATYPE> paddedA(A_finalVolume);
    padMatrixWithZeros(AVec, M, K,
                        paddedA, finalM, finalK);
    memcpy(bufA, paddedA.data(), (paddedA.size() * sizeof(A_DATATYPE)));
  ...
  for (int i = 0; i < B_VOLUME; i++) {
    ...
    /* To consider padding */
        std::vector<B_DATATYPE> paddedB(B_finalVolume);
        padMatrixWithZeros(BVec, K, N,
                        paddedB, finalK, finalN);
  memcpy(bufB, paddedB.data(), (paddedB.size() * sizeof(B_DATATYPE)));
  // memcpy(bufB, BVec.data(), (BVec.size() * sizeof(B_DATATYPE)));
    std::cout << "Matrix has been padded" << std::endl;
    ...
    if (do_verify) {
      ...
      if (do_verify_stochastic) {
        ...
        errors = matmul_common::verify<A_DATATYPE, C_DATATYPE, ACC_DATATYPE>(
            finalM, finalN, finalK, paddedA, paddedB, CVec, verbosity, abs_tol, rel_tol, b_col_maj);
      }
