...
#include "common_small_dimensions.h"
...
int main(int argc, const char *argv[]) {
  ...
  bool do_verify_stochastic =
      (long long)(M1+M2) * N * K > verify_stochastic_threshold;

  if (verbosity >= 1) {
    std::cout << "Matrix size " << (M1+M2) << "x" << K << "x" << N << std::endl;
  }

  int A1_VOLUME = M1 * K;
    int A2_VOLUME = M2 * K;
  int B_VOLUME = N * K;
  int C_VOLUME = (M1+M2) * N;
    int C1_VOLUME = M1 * N;
    int C2_VOLUME = M2 * N;

  size_t A1_SIZE = (A1_VOLUME * sizeof(A_DATATYPE));
    size_t A2_SIZE = (A2_VOLUME * sizeof(A_DATATYPE));
  ...
  size_t C_SIZE = (C_VOLUME * sizeof(C_DATATYPE));
    ...
    size_t C2_SIZE = (C2_VOLUME * sizeof(C_DATATYPE));

  size_t OUT_SIZE = C_SIZE + trace_size;
    size_t OUT1_SIZE = C1_SIZE + trace_size;
    size_t OUT2_SIZE = C2_SIZE + trace_size;

  std::vector<uint32_t> instr_v =
      matmul_common::load_inst
  ...
  auto bo_a1 =
      xrt::bo(device, A1_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(3));
  auto bo_a2 =
      xrt::bo(device, A2_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
  auto bo_b =
      xrt::bo(device, B_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(5));
  auto bo_out1 =
      xrt::bo(device, OUT_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(6));
     auto bo_out2 =
      xrt::bo(device, OUT_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(7));
  ...
  std::vector<A_DATATYPE> AVec(2*A1_VOLUME);
  for (int i = 0; i < 2*A1_VOLUME; i++) {
    // AVec[i] = matmul_common::get_random<A_DATATYPE>();
    AVec[i] = i;
  }
    
  // Split A into A1 and A2
    std::cout << "Splitting matrix A..." << "\n";
    std::vector<A_DATATYPE> AVec_first_half;
    std::vector<A_DATATYPE> AVec_second_half;
    splitMatrix(AVec, (M1+M2), K, AVec_first_half, AVec_second_half);
    std::cout << "Finished splitting matrix A..." << "\n";
    std::cout << "Size of A first half: " << AVec_first_half.size() << "\n";
    std::cout << "Size of A second half: " << AVec_second_half.size() << "\n";
    std::cout << "Size of original A: " << AVec.size() << "\n";
    memcpy(bufA1, AVec_first_half.data(), (AVec_first_half.size() * sizeof(A_DATATYPE)));
    memcpy(bufA2, AVec_second_half.data(), (AVec_second_half.size() * sizeof(A_DATATYPE)));
  ...
  std::vector<C_DATATYPE> CVec_1(C1_VOLUME);
  memset(bufOut1, 0, OUT1_SIZE);

    std::vector<C_DATATYPE> CVec(C_VOLUME);

  char *bufOut2 = bo_out2.map<char *>();
  std::vector<C_DATATYPE> CVec_2(C2_VOLUME);
  memset(bufOut2, 0, OUT2_SIZE);
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    auto run = kernel(opcode, bo_instr, instr_v.size(), bo_a1, bo_a2, bo_b, bo_out1, bo_out2);
    ...
    if (do_verify) {
      // memcpy(CVec.data(), bufOut, (CVec.size() * sizeof(C_DATATYPE)));
        memcpy(CVec_1.data(), bufOut1, (CVec_1.size() * sizeof(C_DATATYPE)));
        memcpy(CVec_2.data(), bufOut2, (CVec_2.size() * sizeof(C_DATATYPE)));
        appendVectors(CVec_1, CVec_2, CVec);
      if (verbosity >= 1) {
