...
int main(int argc, const char *argv[]) {
  ...
  auto bo_a =
      xrt::bo(device, P_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(3));
  auto bo_b =
  ...
  auto bo_out =
     ...
     // Other half of A
    auto bo_p =
      xrt::bo(device, P_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(6));
  ...
  for (int i = 0; i < A_VOLUME; i++) {
    ...
    splitMatrix(AVec, M, K, AVec_first_half, AVec_second_half);
    
    A_DATATYPE *bufA_second_half = bo_p.map<A_DATATYPE *>();
    std::cout << "Finished splitting matrix A..." << "\n";
    std::cout << "Size of first half: " << AVec_first_half.size() << "\n";
    std::cout << "Size of second half: " << AVec_second_half.size() << "\n";
    std::cout << "Size of original A: " << AVec.size() << "\n";
  memcpy(bufA, AVec_first_half.data(), (AVec_first_half.size() * sizeof(A_DATATYPE)));
  memcpy(bufA_second_half, AVec_second_half.data(), (AVec_second_half.size() * sizeof(A_DATATYPE)));
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    auto run = kernel(opcode, bo_instr, instr_v.size(), bo_a, bo_b, bo_out, bo_p);
