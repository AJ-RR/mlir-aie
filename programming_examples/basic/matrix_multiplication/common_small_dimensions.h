...
void add_default_options(po::options_description &desc) {
  desc.add_options()("help,h", "produce help message")(
      ...
      "M1,M1", po::value<int>()->default_value(512), "Half Matrix size of A")(
      "M2,M2", po::value<int>()->default_value(512), "Half Matrix size of A")(
...
int verify(int M, int N, int K, std::vector<Tin> A, std::vector<Tin> B,
  ...
  matmul<Tin, Tout, Tacc>(M, N, K, A, B, CRef, b_col_maj);
    ...
    std::cout<<"Size of Output: "<<C.size()<<"\n";
...
template <typename T>
void reshapeC(const std::vector<T>& matrixC, int targetRows, int targetCols, std::vector<T>& C_reshaped) {
    // Calculate the split point (half the number of rows)
    int currentRows = targetRows / 2;
    int currentColumns = 2*targetCols;
    
    // Temporary matrix
    // Declare and initialize a 2D vector with specified dimensions
    std::vector<std::vector<T>> originalVec(currentRows, std::vector<T>(currentColumns, 0));
    int counter = 0;
    for(int i = 0; i < currentRows; i++){
        for(int j=0; j < currentColumns; j++){
            originalVec[i][j] = matrixC[counter];
            counter++;
        }
    }

    std::cout<<"Completed temp matrix"<<"\n";

   // T reshapedVec[targetRows][targetCols];
    std::vector<std::vector<T>> reshapedVec(targetRows, std::vector<T>(targetCols));
    int rowCounter = currentRows;
    int alreadyIncremented = 1;
    for(int i = 0; i < currentRows; i++){
        int newRow = i;
        for(int j=0; j < currentColumns; j++){
            ...
            // Determine which row in the original matrix we are pulling from
            int newCol = j % targetCols;
            std::cout<<"New row: "<<newRow<<"New col: "<<newCol<<"\n";
            reshapedVec[newRow][newCol] = originalVec[i][j];
        }
        alreadyIncremented = 0;
    }
    std::cout<<"Completed reshaped matrix"<<"\n";
    // Write the 2d matrix back as row major
    for(int i = 0; i < targetRows; i++){
        for(int j=0; j < targetCols; j++){
            T newVlaue = reshapedVec[i][j];
            C_reshaped.push_back(newVlaue);
            std::cout<<reshapedVec[i][j]<<"\n";
        }
    }
}

template <typename T>
void appendVectors(const std::vector<T>& vec1, const std::vector<T>& vec2, std::vector<T>& result) {
    // Clear result to ensure it's empty before appending
    result.clear();
    
    // Insert all elements of vec1 and vec2 into result
