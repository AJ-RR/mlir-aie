...
def my_matmul(
    ...
    def device_body():
        ...
        B_l3l2_fifos = [None] * n_aie_cols          # Only use first 2 mem and shim tiles
        B_l2l1_fifos = [None] * n_aie_cols          # Only use 2 fifos. Each fifo broadcasts to two columns
        ...
        for col in range(2):
            ...
            core_tiles_to_broadcast = [core_tiles[j][col] for j in range(n_aie_rows)] + [core_tiles[j][col+2] for j in range(n_aie_rows)]
            B_l2l1_fifos[col] = object_fifo(
                f"B_L2L1_{col}",
                mem_tiles[col],
                core_tiles_to_broadcast,  # broadcast along one column and the alternate column
                fifo_depth,
                B_l1_ty,
                (
                    [
        ...
        def sequence(A1, A2, B, C1, C2):
            ...
            for tb in range(ceildiv(M1 // m // n_aie_rows, tb_max_n_rows)):
                for pingpong in [0, 1]:
                    # 2*M1 // m // n_aie_rows // tb_max_n_rows
                    ...
                    for col in range(n_aie_cols):
                        ...
                        for tile_row in range(tb_n_rows):
                            ...
                            if col < 2:   # We are using only first two shim and mem tiles for B
                                ...
                                #     |0011    0011    |
                                #      ----------------
                                B_col_offset = (col) * n if not b_col_maj else (col%2) * n * K
                                npu_dma_memcpy_nd(
