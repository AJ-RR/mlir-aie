...
# For now, pass the original A matrix divided by half (M/2 x K). Let the new number of columns be P.
def my_matmul(
    M, K, N, m, k, n, n_aie_cols, dtype_in_str, dtype_out_str, b_col_maj, trace_size,
):
    # Split A in two so P = M / 2
    P = int(M / 2)
    M = int(M / 2) 
    n_aie_rows = 4
    ...
    n_tiles_per_core = (2*M // m) * (N // n) // n_aie_cores
    ...
    def device_body():
        ...
        P_l2_ty = np.ndarray[(m * k * n_A_tiles_per_shim,), np.dtype[dtype_in]]
        B_l2_ty = np.ndarray[(k * n,), np.dtype[dtype_in]]
        C_l2_ty = np.ndarray[(m * n * n_aie_rows,), np.dtype[dtype_out]]
        A_l1_ty = np.ndarray[(m, k), np.dtype[dtype_in]]
        P_l1_ty = np.ndarray[(m, k), np.dtype[dtype_in]]
        ...
        A_l3l2_fifos = [None] * 2  # two columns for first half of A
        A_l2l1_fifos = [None] * n_aie_rows

        # For the other half P
        P_l3l2_fifos = [None] * 2  # two columns for second half of A
        P_l2l1_fifos = [None] * n_aie_rows

        B_l3l2_fifos = [None] * 2  # only two required for B. They will distribute accordingly
        B_l2l1_fifos = [None] * 2
        ...
        for row in range(n_aie_rows):
            A_l2l1_fifos[row] = object_fifo(
                f"A_L2L1_{row}",
                mem_tiles[row%2], # Use only first two mem tiles for first half of A 
                core_tiles[row][0:2],  # broadcast along half of one row
                fifo_depth,
                A_l1_ty,
                [
                    (m // r, r * k),
        ...
        for col in range(2):
            ...
            object_fifo_link(
                A_l3l2_fifos[col],
                [A_l2l1_fifos[row] for row in [col,col+2]],
                [],
                of_offsets,
            )
        ...
        # Input P
        for row in range(n_aie_rows):
            P_l2l1_fifos[row] = object_fifo(
                f"P_L2L1_{row}",
                mem_tiles[(row%2)+2],
                core_tiles[row][2:4],  # broadcast along half of one row
                fifo_depth,
                P_l1_ty,
                [
                    (m // r, r * k),
        ...
        for col in [2,3]:
            P_l3l2_fifos[col-2] = object_fifo(
                f"P_L3L2_{col}",
                shim_tiles[col],
                mem_tiles[col],
                fifo_depth,
            ...
            object_fifo_link(
                P_l3l2_fifos[col-2],
                [P_l2l1_fifos[row] for row in [col-2,col]],
                [],
                of_offsets,
            )
        ...
        # Input B
        # Have only 2 shim and mem tiles instead of n_aie_cols
        for col in range(2):
            ...
            B_l2l1_fifos[col] = object_fifo(
                ...
                ],  # broadcast along alternate columns
                fifo_depth,
                B_l1_ty,
                (
                    [
                        ...
                        for _ in loop:
                            ...
                            for _ in range_(K // k):
                                if col < 2:
                                    ...
                                    elem_in_a = P_l2l1_fifos[row].acquire(
                                        ObjectFifoPort.Consume, 1
                                    )
                                ...
                                if col < 2:
                                    ...
                                    P_l2l1_fifos[row].release(ObjectFifoPort.Consume, 1)
                ...
                for pingpong in [0, 1]:
                    ...
                    for col in range(n_aie_cols):
                        ...
                        npu_dma_memcpy_nd(
                            ...
                            sizes=[tb_n_rows, N // n, m * n_aie_rows, n],
                        ...
                        for tile_row in range(tb_n_rows):
                            if col < 2:
                                ...
                                P_offset = P_block_offset + P_row_offset
                                npu_dma_memcpy_nd(
                                    metadata=P_l3l2_fifos[col%2],
                                    bd_id=bd_id_base + 2 * tile_row + 1,
                                    mem=P,
                                    offsets=[0, 0, 0, P_offset],
                                    sizes=[
                                        N // n // 2,
                                        K // k,
                                        m * n_A_tiles_per_shim,
