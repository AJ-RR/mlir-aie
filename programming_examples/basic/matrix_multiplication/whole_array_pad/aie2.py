...
def my_matmul(
    ...
    # Update if padding was required
    # Modify M such that M is a multiple of (m * n_aie_rows)
    M = ceildiv(M , (m * n_aie_rows)) * (m * n_aie_rows)
    
    # Modify K such that K is a multiple of k
    K = ceildiv(K, k) * k
    
    # Modify N such that N is a multiple of (n * n_aie_cols)
    N = ceildiv(N, (n * n_aie_cols)) * (n * n_aie_cols)
